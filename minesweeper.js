...
export function revealTile (board, tile) {
    ...
    const mines = adjacentTiles.filter(t => t.mines)
    if (mines.length ===0){
...
export function checkWin(board){
    return board.every(row =>{
        return row.every(tile => {
            return (t.status === TILE_STATUSES.NUMBER || (tile.mine && (tile.status
                === TILE_STATUSES.HIDDEN || tile.status === TILE_STATUSES.MARKED))
            )
        })
    })
